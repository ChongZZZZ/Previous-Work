...
/**
 * Run the main UI loop. This function will only return if the UI is exiting.
 */
void ui_run() {
...
*/
void ui_display_space(int space) {
...
//TO DO the board
void ui_display_board(const char* username, int* board, int board_size) {
  // ui_display("HAHA","Trying to print");
  //  Lock the UI
  pthread_mutex_lock(&ui_lock);
  ...
  if (ui_running) {
    ...
    */
    ...
    form_driver(display_form, REQ_NEW_LINE);  //< Newline for spacing
    for (int i = 0; i < rows; i++) {          //< i is for printing rows
      for (int j = 0; j < board_size; j++) {  //< j is for printing columns
        ...
        if (i % 2 == 0) {  //< Print the main columns
          // Display the x, o, or " "
          ui_display_space(board[row_count * board_size + j]);
          // ui_display("HAHA","Trying to print again");

          // Print the lines between each space
          if (j != board_size - 1) form_driver(display_form, '|');

        } else {  //< Print the lines in between
          const char* c = "---";
          while (*c != '\0') {
            form_driver(display_form, *c);
            c++;
    ...
    // Print to the console if the UI is not running
    printf("%s:\n", username);
